...
import { Loader2, Zap, LogIn, Mail } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Usuário será redirecionado para o Google — não precisa fazer nada aqui
    } catch (err: any) {
      toast({
        variant: "destructive",
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-sm">
          ...
          <Button
            type="button"
            variant="outline"
            className="mb-4 h-12 w-full rounded-xl border-white/15 bg-white/5 text-sm font-semibold hover:bg-white/10 gap-3"
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="login-email" className="text-xs text-slate-400">E-mail</Label>
              <Input
                id="login-email"
                type="email"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="login-password" className="text-xs text-slate-400">Senha</Label>
              <Input
                id="login-password"
                type="password"
                placeholder="••••••••"
