...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (!loading && user) {
      ...
      if (cleanEmail === "caioluispeixotos@gmail.com" || user.status === "approved") {
        router.replace("/");
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="mb-6 flex flex-col items-center gap-2">
          ...
          <p className="text-xs text-slate-400 text-center">
            {mode === "login"
              ? "Acesse sua conta ou entre com o Google"
              : "Crie seu acesso. Novos cadastros requerem aprovação do administrador."}
          </p>
        </div>

        {/* Alternate Tabs */}
        <div className="mb-4 flex rounded-xl border border-white/10 bg-white/5 p-1 backdrop-blur-sm">
          <button
            type="button"
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-sm">
          ...
          <Button
            type="button"
            variant="outline"
            className="mb-4 h-12 w-full rounded-xl border-white/15 bg-white/5 text-sm font-semibold hover:bg-white/10 gap-3 text-slate-200"
          ...
          <div className="relative mb-4">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="bg-[#0f1115] px-2 text-slate-500">ou use seu e-mail</span>
            </div>
          </div>
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            ...
            >
              {submitting ? (
                ...
                <>
                  <UserPlus className="h-4 w-4" /> Criar Conta e Solicitar Acesso
                </>
              )}
            </Button>
          </form>
